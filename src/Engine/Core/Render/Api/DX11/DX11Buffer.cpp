...
namespace Engine {
    DX11Buffer::DX11Buffer(ComPtr<ID3D11Device> d3dDevice, D3D11_USAGE usage, D3D11_BIND_FLAG bindFlags, UINT cpuAccessFlags, UINT size, UINT strides, const void* data)
        : m_strides(strides), m_offset(0), m_isUpdatable(usage == D3D11_USAGE_DYNAMIC) {
    ...
    void DX11Buffer::Update(ComPtr<ID3D11DeviceContext> d3dContext, UINT size, const void* data) {
        if (!m_isUpdatable) {
            throw EngineException("[DX11Buffer] DX11Buffer::Update() failed. The buffer cannot be updated");
        }
        ...
        memcpy(d3dMappedSubresources.pData, data, size * m_strides);
